...
describe("background", () => {
  ...
  it("returns a clear error when scanned option-backed fields have sparse options", async () => {
  ...
  it("returns a clear error when scanned non-choice fields have other-option metadata", async () => {
    const listener = await loadBackgroundWithChrome({
      tabs: {
        query(_queryInfo: chrome.tabs.QueryInfo, callback: (tabs: chrome.tabs.Tab[]) => void) {
          callback([tabWithUrl("https://docs.google.com/forms/d/e/form-id/viewform")]);
